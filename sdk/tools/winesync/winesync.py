...
class wine_sync:
    ...
    def sync_wine_commit(self, wine_commit, in_staging, staging_patch_index):
        ...
        if (warning_message != ''):
            ...
            warning_message += (
                f"""\nDo not forget to run\n    git diff HEAD^ \':(exclude)sdk/tools/winesync/{patch_file_name}\' > sdk/tools/winesync/{patch_file_name}\nafter your correction and then\n    git add sdk/tools/winesync/{patch_file_name}\nbefore running "git commit --amend\""""
                if in_staging
                else f'\nYou can see the details of the wine commit here:\n    https://source.winehq.org/git/wine.git/commit/{str(wine_commit.id)}\n'
            )
